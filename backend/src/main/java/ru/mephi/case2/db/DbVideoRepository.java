...
public class DbVideoRepository implements VideoRepository {
    ...
    public List<UrlInfo> getUrlsWithPlatforms() {
        String sql = "SELECT u.id, u.url, p.name AS platform_name FROM urls u JOIN platforms p ON u.platform_id = p.id";
    ...
    public void saveVideoStats(Map<String, Long> urlViewsMap,
